#!/usr/bin/env bash
# =============================================================================
# run_experiment_heavy.sh — O1 Experiment with TC-BPF instrumentation
...
#
# Usage: sudo bash run_experiment_heavy.sh [DURATION] [RUN_ID]
# =============================================================================
set -u
...
incus exec gnb-ue -- start-stop-daemon --start --background \
    ...
    -- -c 10.41.0.1 -p 5201 -u -b 50M -t $((DURATION+60)) -B ${UE1_IP}
ok "eMBB: iperf3 UDP 50 Mbps → UPF1"

# URLLC
SIPP_RATE=8; SIPP_LIMIT=20; SIPP_TOTAL=$((SIPP_RATE*(DURATION+60)))
...
ok "URLLC: sipp 8 calls/s → Asterisk on UPF2"
